...
var sensors = map[string]sensorDef{
	"room_t":      {2, 0, 0, 1},
	"room_0_move": {2, 1, 0, 0},
	// "room_0_light": {2, 2, 0, 0},
	"room_1_move": {3, 1, 0, 0},
	// "room_1_light": {3, 2, 0, 0},
	"room_2_move": {0, 1, 0, 0},
	// "room_2_light": {0, 2, 0, 0},
	"room_3_move": {1, 1, 0, 0},
	// "room_3_light": {1, 2, 0, 0},
	"out_cond": {10, 20, 0, 0},
	"out_t":    {10, 20, 0, 1},
	"out_wind": {10, 20, 0, 5},
	"out_wsp":  {10, 20, 0, 6},
...
func makeBar(val float64, max float64) string {
	idx := int(math.Floor(val / max * float64(len(bar))))
...
func MakeStatus(conf data.HashMap) (string, string) {
	...
	outp := fmt.Sprintf("%s %s %s%.f : %s : %s%s%s%s : %.f%%/%.f%%/%.f%%",
		...
		// makeBar(256-vals["room_0_light"], 256),
		makeFlag(vals["room_1_move"]),
		// makeBar(256-vals["room_1_light"], 256),
		makeFlag(vals["room_2_move"]),
		// makeBar(256-vals["room_2_light"], 256),
		makeFlag(vals["room_3_move"]),
		// makeBar(256-vals["room_3_light"], 256),
		vals["batt_0"],
		vals["batt_1"],
		vals["batt_2"])
